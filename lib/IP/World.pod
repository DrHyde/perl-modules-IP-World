...
The install process for IP::World runs I<maint_ip_world_db> to check
whether the database included in the distribution is current with the 
data sources.

If I<maint_ip_world_db> is run with a C<-d> or C<--dual> switch, it 
checks/writes two files named ipworld.be and ipworld.le, rather than 
one file named ipworld.dat.  This is mostly for the author in making 
new distributions.
...
This uses one of the two data sources that IP-World uses.  It also 
will work with MaxMind's licensed, fee-based databases which include 
more data.

Unfortunately if you want this package to operate at the speeds 
shown in L</PERFORMANCE>, you have to download a C library from the 
internet and install it from the command line.  Someone should tell 
them about XS modules so that the whole package can be obtained via 
CPAN or CPANPLUS...  
